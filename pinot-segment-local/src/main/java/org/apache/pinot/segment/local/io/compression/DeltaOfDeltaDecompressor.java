...
/**
 * Implementation of {@link ChunkDecompressor} for delta of delta compression with LZ4.
 * This decompressor reconstructs the original integer sequence from LZ4 compressed delta of delta encoded values.
 */
class DeltaOfDeltaDecompressor implements ChunkDecompressor {

  static final DeltaOfDeltaDecompressor INSTANCE = new DeltaOfDeltaDecompressor();
  ...
  public int decompress(ByteBuffer compressedInput, ByteBuffer decompressedOutput) throws IOException {
    // Read type flag
    byte typeFlag = compressedInput.get();
    if (typeFlag == INT_FLAG) {
      return decompressForInt(compressedInput, decompressedOutput);
    } else if (typeFlag == LONG_FLAG) {
      return decompressForLong(compressedInput, decompressedOutput);
    } else {
      throw new IOException("Invalid type flag: " + typeFlag);
    }
  }

  private int decompressForInt(ByteBuffer compressedInput, ByteBuffer decompressedOutput) throws IOException {
  ...
  private int decompressForLong(ByteBuffer compressedInput, ByteBuffer decompressedOutput) throws IOException {
    // Get number of longs
  ...
  public int decompressedLength(ByteBuffer compressedInput) {
    // Skip type flag
    byte typeFlag = compressedInput.get(compressedInput.position());
    ...
    if (typeFlag == INT_FLAG) {
      ...
      throw new IllegalStateException("Invalid type flag: " + typeFlag);
    }
  }
} 