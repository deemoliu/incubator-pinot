...
/**
 * Implementation of {@link TableUpsertMetadataManager} backed by trie-based partition managers.
 * This provides memory-efficient primary key storage through prefix sharing.
 */
@ThreadSafe
public class TrieTableUpsertMetadataManager extends BaseTableUpsertMetadataManager {
  private static final Logger LOGGER = LoggerFactory.getLogger(TrieTableUpsertMetadataManager.class);
  ...
  public BasePartitionUpsertMetadataManager getOrCreatePartitionManager(int partitionId) {
    return _partitionMetadataManagerMap.computeIfAbsent(partitionId, k -> {
      if (_context.isEnableDeletedKeysCompactionConsistency()) {
        throw new UnsupportedOperationException(
            "TrieTableUpsertMetadataManager does not support enableDeletedKeysCompactionConsistency");
      }
      return new TriePartitionUpsertMetadataManager(_tableNameWithType, k, _context);
    });
  }
