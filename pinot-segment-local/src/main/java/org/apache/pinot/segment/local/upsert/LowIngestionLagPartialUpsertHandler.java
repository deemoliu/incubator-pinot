 ...
 */
public class LowIngestionLagPartialUpsertHandler {
  ...
  public LowIngestionLagPartialUpsertHandler(Schema schema, Map<String, UpsertConfig.Strategy> partialUpsertStrategies,
  ...
  public GenericRow diskIOImprovedMerge(GenericRow previousRecord, GenericRow newRecord) {
    if (_defaultPartialUpsertMerger.equals(UpsertConfig.Strategy.IGNORE)) {
      // if default is ignore, it keeps the old record as base, only access the not nil values from previous.
      for (String column : _column2Mergers.keySet()) {
        if (!_primaryKeyColumns.contains(column)) {
          if (!newRecord.isNullValue(column)) {
            if (previousRecord.isNullValue(column)) {
              previousRecord.putValue(column, newRecord.getValue(column));
              previousRecord.removeNullValueField(column);
            } else if (!_comparisonColumns.contains(column)) {
              PartialUpsertMerger merger = _column2Mergers.get(column);
              previousRecord.putValue(column,
                  merger.merge(previousRecord.getValue(column), newRecord.getValue(column)));
            }
          }
        }
    ...
    } else if (_defaultPartialUpsertMerger.equals(UpsertConfig.Strategy.OVERWRITE)) {
      // if used overwrite, it keep the new record.
      for (String column : _column2Mergers.keySet()) {
        if (!_primaryKeyColumns.contains(column)) {
          if (!previousRecord.isNullValue(column)) {
            if (newRecord.isNullValue(column)) {
