...
public class HeapUsageEstimateCommand extends AbstractBaseAdminCommand implements Command {
  ...
  @CommandLine.Option(names = {"-columnStats"}, required = true, description = "Primary key columns bytes")
  private int _columnStats = 8;
  ...
  public HeapUsageEstimateCommand setColumnStats(int columnStats) {
  ...
  public String run()
    ...
    // estimated key space
    // estimated value space, it contains <segmentName, DocId, ComparisonValue>
    List<String> primaryKeys = schema.getPrimaryKeyColumns();
    int bytesPerKey = 0;
    int bytesPerValue = 64; // segmentName, DocId, ComparisonValue
