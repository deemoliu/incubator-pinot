...
public class UpsertConfigTest {
  ...
  public void testGetTtlInMs() {
    Assert.assertEquals(new UpsertTTLConfig(TimeUnit.DAYS, 1).getTtlInMs(), 86400000);
    Assert.assertEquals(new UpsertTTLConfig(TimeUnit.HOURS, 1).getTtlInMs(), 3600000);
    Assert.assertEquals(new UpsertTTLConfig(TimeUnit.MINUTES, 1).getTtlInMs(), 60000);
    Assert.assertEquals(new UpsertTTLConfig(TimeUnit.SECONDS, 1).getTtlInMs(), 1000);
  }
}
